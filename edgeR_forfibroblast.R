...
#only do per-family analysis
#CHANGE PER FAMILY 
metadata <- metadata[ (metadata$family=="N030"), ]
...
#groups<- c("mild", "unaffected", "unaffected")
...
res <- topTags(lrt, n = nrow(dgList), sort.by = "PValue")

keep <- out$table$FDR <= 0.05 & abs(out$table$logFC) >= 0
keep2=out[keep,]
...
#dont really need to run this part if you use upsetR plot 
############################
...
write.csv(frame, file = "results2.csv")

###if its just 2 comparisons you can just use the intersect functions after maually compiling 
###dont need functions below those are for 2+ comparisons

venn=read.csv("N036vN030goandkeggORAoutputs.csv", header = FALSE)
intersect(venn$V1, venn$V3) #goterms
intersect(venn$V2, venn$V4) #keggterms 
